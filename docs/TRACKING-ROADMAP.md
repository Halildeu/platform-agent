...
| AG-013 | Windows local user adapter | `DONE` | Local users read-only listelenir. | `scripts/test/windows-live.ps1` Parallels Windows 11: `diagnose local-users` Administrator, DefaultAccount, Guest, WDAGUtilityAccount, halilkocoglu listesi dondu. |
...
| ID | Is | Durum | Acceptance criteria | Evidence |
|---|---|---|---|---|
| WEB-001 | `mfe-endpoint-admin` scaffold | `TODO` | Vite/MF app olusur, shell remote olarak taninir. | Yok |
| WEB-002 | Shell route | `TODO` | `/admin/endpoints` route'u protected olarak acilir. | Yok |
| WEB-003 | Endpoint list | `TODO` | Hostname, OS, state, lastSeen, agent version listelenir. | Yok |
| WEB-004 | Endpoint detail tabs | `TODO` | Inventory, Users, Files, Commands, Audit tablari var. | Yok |
| WEB-005 | Command picker | `TODO` | Freeform command yok; yalniz whitelist action form'lari var. | Yok |
