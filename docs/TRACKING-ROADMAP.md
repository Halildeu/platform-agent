...
| Password reset (local) | `IN_PROGRESS` | AG-042 source-side Windows SAM adapter acildi (`LOCK_USER_LOGIN`, `UNLOCK_USER_LOGIN`, `CHANGE_LOCAL_PASSWORD`); backend secret-delivery, live destructive E2E ve pilot gate ayri kalir. Domain/M365 reset ayri track. |
...
| Destructive command saga | `IN_PROGRESS` | Backend dual-control + audit hatti vardir; AG-042 local-user adapter source-side eklenir. Partial failure/rollback ve live destructive pilot kaniti ayri gate. |
...
| AG-016 | Password reset local adapter | `IN_PROGRESS` | SAM-only `CHANGE_LOCAL_PASSWORD` agent adapter'i AG-042 ile source-side acilir; payload secret'i result/audit'e donmez. Backend secret-delivery + live destructive E2E + pilot gate olmadan tamamlandi sayilmaz. | `internal/users/local_windows.go`, `internal/commands/executor_test.go`, `go test ./...`, `./scripts/build/windows-package.sh` |
...
| AG-021 | Windows identity inventory | `IN_PROGRESS` | Agent `COLLECT_INVENTORY` sonucuna read-only identity block ekler: domain/workgroup, `PartOfDomain`, `dsregcmd /status` join state, domain reachability probe, tenant/device id hashleri ve OS build bilgisi. Password reset/user disable/file access yok. | `internal/identity`, `internal/inventory/inventory.go`, issue #16 |
...
| AG-029 | Signed self-update | `IN_PROGRESS` | Agent approved release catalog uzerinden gelen UPDATE_AGENT komutunu local signer policy, Authenticode evidence, version bind, stage/activate ve backend heartbeat kanitlariyla uygular; tek lab cihazi genis rollout iddiasi uretmez. | PR #74 MERGED: Parallels W11 `HALILKOOLUB735` local smoke 0.1.1-lab.3 -> 0.1.2-lab.2 PASS, command `f34f9c58-c0d6-4948-bb18-ba1415e2224a` SUCCEEDED, heartbeat `0.1.2-lab.2 ONLINE`; multi-device checklist `docs/AG-029-multi-device-acceptance-checklist.md`. |
| AG-042 | Local user destructive action adapter | `IN_PROGRESS` | Windows-only backend canonical `LOCK_USER_LOGIN`, `UNLOCK_USER_LOGIN` ve `CHANGE_LOCAL_PASSWORD` komutlari NetUserSetInfo tabanli adapter'e iner; domain/M365 hesabi degil local SAM hedeflenir; secret payload fail-closed/redacted kalir. | platform-agent#79; `internal/protocol/types.go`, `internal/users/local_windows.go`, `internal/commands/executor.go`, `internal/inventory/inventory.go`, `go test ./...`, Windows cross-compile |
...
| E - Password reset / file access | `OPS-001`, `BE-017`, `AG-016`, `AG-042`, `BE-018A`, `BE-018B`, `AG-017` | RBAC, audit integrity, saga, pilot ve identity discovery olmadan acilmaz |
